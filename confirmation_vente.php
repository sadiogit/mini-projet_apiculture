...
<body>
	...
	<div class="row" style="margin-right: 10%; margin-left: 10%; ">
			...
			<form action="<?php $_SERVER['PHP_SELF'] ?>" method="get">
			  ...
			  <!-- Debut tableau --> 
				...
				<tr>
					...
					<th><center>Numero Cmd</center></th>
					...
					<?php
						...
						{
					?>
							<th><center>Voir l'etat</center></th>
					<?php
				...
				<?php
					...
					if ((isset($_GET['date_debut']))&&(isset($_GET['date_fin']))&&(!empty($_GET['date_debut']))&&(!empty($_GET['date_fin'])))
					{
						...
						$req=$pdo->query("select distinct cmd.Id_cmd,cmd.Id_cli,cmd.Mont_total_cmd,cmd.enreg_par,cmd.etat_cmd from cmd,detail_cmd,stock_prod where cmd.Id_cmd=detail_cmd.Id_cmd and detail_cmd.id_stock_prod=stock_prod.id_stock_prod and stock_prod.id_magasin=1 order by cmd.Id_cmd");
						...
						?>
							<td> <input type=submit class="btn btn-primary" value="Confirmer"  > </td>
						<?php

						}else
						{
						?>
								<td> <input type=submit class="btn btn-primary" value="Voir"  > </td>
						<?php

						}
											
		               	 echo "<input type=hidden name=detail_confirmation_vente value=$ta[0] >";
			 ?>
		...
		<?php									
					...
					{
						$req=$pdo->query("select Id_cmd,Id_cli,Mont_total_cmd,cmd.enreg_par,cmd.etat_cmd from cmd order by Id_cmd desc");
